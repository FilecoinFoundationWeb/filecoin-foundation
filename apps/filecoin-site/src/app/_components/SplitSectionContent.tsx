  ...
  return (
    <div className={clsx('grid grid-cols-6', gap)}>
      <div className="col-span-6 lg:col-span-2 lg:pr-8">
        <Heading
          tag="h3"
          className="font-heading text-lg font-light tracking-wide text-balance text-[var(--color-text-heading-muted)] md:text-2xl"
        >
          {title}
        </Heading>
      </div>
      ...
      <div className={clsx('col-span-6 flex flex-col lg:col-span-4', gap)}>
        <div className="space-y-6">
          {descriptionArray.map((item, index) => (
            <p
              key={index}
              className="font-sans text-lg/7 font-light text-balance md:text-2xl/9"
            >
              {item}
            </p>
          ))}
