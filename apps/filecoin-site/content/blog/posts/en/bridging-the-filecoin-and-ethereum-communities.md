...
- **Infura’s Filecoin Network API** – We already mentioned Infura’s groundbreaking IPFS API. Infura is now offering select users free, invite-only access to a new Filecoin API beta. This beta will provide developers with easy access to the Filecoin mainnet for building applications, apps, services, or features on top of the Filecoin network. Users can [register with Infura](https://infura.io/register) to sign up for access to the beta.
