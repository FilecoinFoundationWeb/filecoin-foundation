...
In a leader election, every miner draws a random seed provided by the [drand beacon](/blog/filecoin-features-distributed-randomness-leader-elections/). Miners then perform an _Election Proof_ function on this random seed. If the value generated by the Election Proof function is valid, then the miner is eligible to mine a block. When miners generate new blocks during EC, these blocks come together to form “tipsets,” families of blocks mined in the same epoch and connected to the same parent blocks from a previous epoch.
