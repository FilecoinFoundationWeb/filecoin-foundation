...
正如我们在博客文章[加密经济结构](/blog/filecoin-s-cryptoeconomic-constructions/)中所讨论的那样，有时当您处理海量数据时，很难通过互联网传输这些数据以进行保管（非常昂贵和耗费时间）。对于PB级或者更大的数据集，通常最明智的方法是用硬盘来传输数据。Filecoin协议和项目拥有支持所谓的离线数据传输的工具与结构。
