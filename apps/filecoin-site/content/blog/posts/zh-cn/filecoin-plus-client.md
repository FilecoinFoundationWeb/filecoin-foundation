...
为了最大化 Filecoin 网络中可支持的有用存储量，Filecoin Plus（Fil+）被引入作为一层社会信任机制。
客户端可通过分配者（allocators）申请并获得一种新的资源——DataCap（数据配额）。
获得 DataCap 后，客户端可以使用该配额与存储提供者达成存储交易；存储提供者因存储经验证的有用数据，将获得更高的区块奖励激励。
