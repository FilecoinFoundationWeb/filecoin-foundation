...
export function convertMarkdownToDigestArticleData(data: Record<string, any>) {
  ...
  const description = generateAndCleanDescription(data.content)
...
function generateAndCleanDescription(content: string) {
  return cleanText(
    removeMarkdown(content).substring(0, CONTENT_PREVIEW_CHARACTER_LENGTH),
  )
}
